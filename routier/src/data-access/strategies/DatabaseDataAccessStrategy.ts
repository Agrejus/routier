...
export class DatabaseDataAccessStrategy<T extends {}> extends DataAccessStrategyBase<T> implements IDataAccessStrategy<T> {
    ...
    bulkPersist(_: CollectionOptions, event: DbPluginBulkPersistEvent<T>, done: PluginEventCallbackPartialResult<ResolvedChanges<T>>) {
        super._bulkPersist(event, done);
    }

    query<TShape>(_: CollectionOptions, event: DbPluginQueryEvent<T, TShape>, done: PluginEventCallbackResult<TShape>) {
