  ...
  globalThis.sqlite3Worker1Promiser.defaultConfig = {
    worker: function() {
      return new Worker(new URL(
        /* @vite-ignore */
        "/lab/assets/sqlite3-worker1-9JnzZYSX.js",
        self.location.href
      ), { type: "module" });
    },
  ...
  */
  async function sqlite3InitModule(moduleArg = {}) {
    ...
    function findWasmBinary() {
      ...
      return new URL("/lab/assets/sqlite3-BVKGSWc-.wasm", self.location.href).href;
    }
        ...
        const finalThen = function(arg) {
          ...
          if (config.wasmUtilTarget) {
            ...
            if (!tgt.alloc && arg.instance.exports.malloc) {
              const exports$1 = arg.instance.exports;
              tgt.alloc = function(n) {
                return exports$1.malloc(n) || toss("Allocation of", n, "bytes failed.");
              };
        ...
        opfsUtil.traverse = async function(opt) {
          ...
          (async function callee(dirHandle, depth) {
            for await (const handle2 of dirHandle.values()) if (false === opt.callback(handle2, dirHandle, depth)) return false;
              ...
              if (false === await callee(handle2, depth + 1)) break;
            }
          })(opt.directory, 0);
        };
        ...
        opfsUtil.createVfsState = function() {
          ...
          opfsVfs.bindVfs = function(ioMethods, callback) {
            ...
            return new Promise(function(promiseResolve_, promiseReject_) {
              ...
              const opfsAsyncProxyUrl = new URL("/lab/assets/sqlite3-opfs-async-proxy-D_xnb1D8.js", self.location.href);
  ...
  const loadModule = () => {
    if (modulePromise == null) {
      modulePromise = sqlite3_bundler_friendly_default();
    }
    return modulePromise;
  };
  const loadPool = async (storage) => {
    const sqlite3 = await loadModule();
    if (storage === "memory") {
      return null;
    }
    if (typeof sqlite3.installOpfsSAHPoolVfs !== "function") {
      throw new Error("This build of @sqlite.org/sqlite-wasm cannot install the OPFS SAH pool VFS. Use `wasmDriver({ storage: 'memory' })` if persistence is not required.");
    }
    if (poolPromise == null) {
  ...
  const openDatabase = async (databaseName, storage) => {
    ...
    const database = pool == null ? new sqlite3.oo1.DB(":memory:") : new pool.OpfsSAHPoolDb(`/${databaseName}`);
  ...
  const queryStatement = (database, sql, params) => {
  ...
  const executeStatement = (database, sql, params) => {
    if (params.length === 0) {
      database.exec({
        sql
      });
  ...
  const handle = async (request) => {
    ...
    if (request.kind === "delete") {
      ...
      pool == null ? void 0 : pool.unlink(`/${request.databaseName}`);
      return [];
    }
